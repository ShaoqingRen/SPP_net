...
#pragma omp parallel for
				...
				{   
					...
					problem sub_prob_parr;
					sub_prob_parr.x = sub_prob.x;
					sub_prob_parr.l = sub_prob.l;
					sub_prob_parr.n = sub_prob.n;
					sub_prob_parr.bias = sub_prob.bias;
					sub_prob_parr.y = Malloc(double,sub_prob.l);
					...
					int kt=0;
					for(; kt<si; kt++)
						sub_prob_parr.y[kt] = -1;
					for(; kt<ei; kt++)
						sub_prob_parr.y[kt] = +1;
					for(; kt<sub_prob_parr.l; kt++)
						sub_prob_parr.y[kt] = -1;

					train_one(&sub_prob_parr, param, w, weighted_C[it], param->C);

					for(int j=0;j<w_size;j++)
						model_->w[j*nr_class+it] = w[j];

					free(w);
					free(sub_prob_parr.y);
				}
			}
